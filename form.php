...
//Validar nome
  if (empty($_POST["nome"])) {
    ...
    elseif (strlen($_SESSION['nome']) > 20) {
      $erro['nomeErr'] = "- Nome deve ser inferior a 20 caracteres";
    }
  }
//Validar sobrenome
  if (empty($_POST["snome"])) {
    ...
    elseif (strlen($_SESSION['snome']) > 20) {
      $erro['snomeErr'] = "- Sobrenome deve ser inferior a 20 caracteres";
    }
  }
//Validar email
...
//Validar senha
  if (empty($_POST["pwd"])) {
    ...
    elseif (strlen($_POST["pwd"]) > 30) {
      $erro['pwdErr'] = "- Sua senha deve conter no máximo 30 caracteres!";
    }
    elseif(!preg_match("#[0-9]+#",$_SESSION['pwd'])) {
      $erro['pwdErr'] = "- Sua senha deve conter no mínimo uma letra maiúscula, uma letra minúscula e um número!";
    }
...
//Validar endereço
  if (empty($_POST["end"])) {
    ...
    if (strlen($_SESSION['end']) > 100) {
      $erro['endErr'] = "- Endereço deve ser inferior a 100 caracteres";
    }
  }
//Validar estado
